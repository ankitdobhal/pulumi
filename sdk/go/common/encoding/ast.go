...
func (f *FileAST) SetConfig(keyPath, key string, value config.Value) error {
	if f.ast == nil {
		return nil
	}
	...
	for _, v := range node.Values {
		column = v.GetToken().Position.Column // TODO: not sure if this will work
		if v.Key.String() == key {
	...
	secureMV := func() *ast.MappingValueNode {
		...
		// TODO: calculate column offsets
		v := token.New(value.EncryptedValue(), value.EncryptedValue(), &token.Position{Column: column})
		if value.Secure() {
